...
for i in range(gnss):
    #seleccion de padres
    cndts_par = selec_padres(pob_ori)
    if cndts_par == 0:
        print('Lo sentimos, la aptitud normalizada utilizada como como parametro en nuestro proyecto,\n '
              'presenta problemas con el numero de generaciones solicitado, por favor,\n'
              'introduce un numero de generaciones menor.')
        valor_ver = 0
        break
    else:
        #cruza
        ...
        #reemplazo
        pob_fin = reemplazo(pob_ori, descn)
        pob_ori = pob_fin
...
if valor_ver==1:
    ...
    plt.show()
    print('\n')
    input('Presiona Enter para mas informacion...')
    print('\n')
    promedios_n = []
    ...
    print('Tambien se puede notar que a medida que pase el tiempo el promedio de fitnes en la poblacion va descendiendo \n'
          'esto quiere decir que mientras mas pase el tiempo, las futuras generaciones seran cada vez mas aptas dados \n'
